...
async def declare_impl(network, contract_name, signer, max_fee):
    ...
    """
    print(f"Declaring implementation {contract_name}...")
    ...
    if class_hash_exists(class_hash, network):
        print(f"Implementation with hash {padded_hash} already exists")
    else:
        account = await Account(signer, network, watch_mode="track")
        declared_hash = await account.declare(contract_name, max_fee=max_fee, watch_mode="track")
        if padded_hash != declared_hash:
            raise Exception(
                f"Declared hash {declared_hash} does not match expected hash {padded_hash}"
            )
        print(f"Implementation declared with hash {padded_hash}")
